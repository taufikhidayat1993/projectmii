...
class Breadcrumb
{
  ...
  * @author Ibnu Daqiqil Id
  **/
 public function show($home_site ="home", $id = "page-breadcrumb"  ) 
 {
  $ci = &get_instance();
  $site = $home_site;
  ...
  if ($breadcrumbs && count($breadcrumbs) > 0) {
   $out .= '<li><a class="pathway" href="' . base_url() .'"/>'. $site . '</a>   <i class="fa fa-circle"></i></li>';
   $i=1;
   $sep='';
   foreach ($breadcrumbs as $crumb): 
 
    if ($i != count($breadcrumbs)) {
     $out .= $sep . '<li><a class="pathway" href="' .site_url($crumb['uri']). '">'. $crumb['title'] .'</a>   <i class="fa fa-circle"></i></li>';
    } else {
